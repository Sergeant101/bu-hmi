...
namespace NGO.Elements.Special.BrakeScheme
{
    ...
    /// </summary>
    public partial class AirBrakeControl : UserControl
    {
        public AirBrakeControl()
        {
            InitializeComponent();
        }

        public static DependencyProperty openProperty =
            DependencyProperty.Register("open", typeof(bool), typeof(AirBrakeControl), new FrameworkPropertyMetadata(false, FrameworkPropertyMetadataOptions.AffectsRender, callback));
        ...
        private static void callback(DependencyObject dep_obj, DependencyPropertyChangedEventArgs dp_event_args)
        {
            ...
            var cntrl = (AirBrakeControl)dep_obj;
            if (val!=null)
            {
                if (val)
                {
