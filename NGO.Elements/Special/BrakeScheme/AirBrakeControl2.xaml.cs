...
namespace NGO.Elements.Special.BrakeScheme
{
    ...
    /// </summary>
    public partial class AirBrakeControl2 : UserControl
    {
        public AirBrakeControl2()
        {
            InitializeComponent();
        }

        public static DependencyProperty openProperty =
            DependencyProperty.Register("open", typeof (bool), typeof (AirBrakeControl2), new FrameworkPropertyMetadata(false, FrameworkPropertyMetadataOptions.AffectsRender, callback));
        ...
        private static void callback(DependencyObject dep_obj, DependencyPropertyChangedEventArgs dp_event_args)
        {
            ...
            var cntrl = (AirBrakeControl2) dep_obj;
            if (val!=null)
            {
                if (val)
                {
