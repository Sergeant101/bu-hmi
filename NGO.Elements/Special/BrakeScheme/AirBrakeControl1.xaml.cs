...
namespace NGO.Elements.Special.BrakeScheme
{
    ...
    /// </summary>
    public partial class AirBrakeControl1 : UserControl
    {
        public AirBrakeControl1()
        {
            InitializeComponent();
        }

        public static DependencyProperty openProperty =
            DependencyProperty.Register("open", typeof (bool), typeof (AirBrakeControl1), new FrameworkPropertyMetadata(false, FrameworkPropertyMetadataOptions.AffectsRender, callback));
        ...
        private static void callback(DependencyObject dep_obj, DependencyPropertyChangedEventArgs dp_event_args)
        {
            ...
            var cntrl = (AirBrakeControl1) dep_obj;
            if (val!=null)
            {
                if (val)
                {
